...
对于 **同步块的实现** 使用了(插入指令)**monitorenter和monitorexit指令**(完成对monitor对象的操作)，而　**同步方法** 则是依靠方法**修饰符上的ACC_SYNCHRONIZED**来完成的。无论采用哪种方式，其本质是 **对一个对象的监视器（monitor）进行获取，而这个获取过程是排他的，也就是同一时刻只能有一个线程获取到由 synchronized 所保护对象的监视器**。
